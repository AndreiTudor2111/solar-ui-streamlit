# -*- coding: utf-8 -*-
"""
Created on Sun Jun 22 00:59:37 2025

@author: ostac
"""

# Model12.py
# Script de antrenare modele LSTM, GRU și Deep GRU pe datele utilizatorului și salvarea celui mai bun model ca 12.keras
...
def load_and_prepare_sequences(path, target, window=12, shift=12):
    """
    Încarcă Stan.csv, folosește toate coloanele (excluzând target-ul) ca features.
    """
...
def main():
    parser = argparse.ArgumentParser(description="Antreneaza modele ML pe datele utilizatorului")
    parser.add_argument('--user_hash', required=True, help='Hash-ul email-ului utilizatorului')
    parser.add_argument('--base_dir', default=r"C:\Users\ostac\OneDrive\Desktop\firestore\users", help='Director root utilizatori')
    ...
    if not os.path.exists(stan_path):
        raise FileNotFoundError(f"Fisierul Stan.csv nu exista pentru utilizatorul {args.user_hash}")

    target = 'will_charge'

    # Încarcă datele și pregătește secvențe
    ...
    # Alege cel mai bun model după ROC AUC
    best_model, best_score, best_name = max(models_info, key=lambda x: x[1])
    best_path = os.path.join(user_dir, '12.keras')
