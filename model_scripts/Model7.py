# -*- coding: utf-8 -*-
"""
Created on Sun Jun 22 21:01:27 2025

@author: ostac
"""

# Model7zile.py — Predicție binară la 7 zile în viitor (shift=336)
...
def load_and_prepare_sequences(path, target, window=72, shift=336):
...
def main():
    parser = argparse.ArgumentParser(description="Antrenează modele ML pentru predicție 7 zile")
    ...
    parser.add_argument('--shift', type=int, default=336)
    ...
    if not os.path.exists(stan_path):
        raise FileNotFoundError(f"Fișierul Stan.csv nu există pentru utilizatorul {args.user_hash}")
    ...
    best_path = os.path.join(user_dir, '7.keras')
